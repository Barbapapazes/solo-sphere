...
const github = useAppConfig().portfolio.github
...
<template>
  <nav class="space-y-6 divide-y divide-zinc-200 dark:divide-zinc-800">
    ...
    <ProseNavGroup v-if="resources" class="pt-6" icon="i-heroicons-inbox-20-solid">
      ...
      <div class="space-y-2">
        <ProseNavGroupLink :is="NuxtLink" v-for="resource in resources" :key="resource.name" :to="resource.url" target="_blank" rel="noopener" class="flex items-start gap-2">
    ...
    <ProseNavGroup class="pt-6" icon="i-heroicons-user-group-20-solid">
      ...
      <div class="space-y-2">
        <ProseNavGroupLink :is="NuxtLink" v-for="link in communityLinks" :key="link.name" :to="link.to" target="_blank" rel="noopener" class="flex items-start gap-2">
          <div class="flex items-center gap-2">
